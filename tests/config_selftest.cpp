...
// Host-only (no CUDA); exercises gpu_cr_config_detail::load() directly so
...
const GpuCrBufConfig& gpu_cr_config() {
    static GpuCrBufConfig cfg = gpu_cr_config_detail::load(25UL << 30, 1UL << 30);
    return cfg;
}

static const size_t SHM8 = 8UL << 30;   // an "SHM8 fleet build" default
static const size_t STG1 = 1UL << 30;
...
int main() {
    ...
    clear_env();
    GpuCrBufConfig c = gpu_cr_config_detail::load(SHM8, STG1);
    ...
    c = gpu_cr_config_detail::load(SHM8, STG1);
    ...
    c = gpu_cr_config_detail::load(SHM8, STG1);
    CHECK(c.shm_deferred && c.shm_size == GPU_CR_SHM_FLOOR, "SHM=0 -> deferred at 64MiB floor");
